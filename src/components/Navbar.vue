<template>
  <header class="sticky top-0 z-50 transition-all duration-300 border-b border-gray-100 dark:border-gray-800"
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-16">
        ...
        <nav class="hidden lg:flex items-center space-x-1">
          <a href="#"
            class="px-3 py-2 rounded-md text-sm font-medium text-gray-700 hover:text-embassy-green hover:bg-green-50 transition-colors">
            {{ t('menu.home') }}
          </a>
          ...
          <div class="relative group">
            <button
              class="flex items-center gap-1 px-3 py-2 rounded-md text-sm font-medium text-gray-700 hover:text-embassy-green hover:bg-green-50 transition-colors">
              {{ t('menu.ambassade.title') }}
              <svg class="w-4 h-4 transition-transform group-hover:rotate-180" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            <div
              class="absolute left-0 mt-2 w-56 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top-right z-50">
        ...
        <div class="flex items-center gap-3">
          ...
          <button @click="isMenuOpen = !isMenuOpen"
            class="lg:hidden p-2 rounded-md text-gray-600 hover:text-embassy-green hover:bg-gray-100 focus:outline-none">
      ...
      <div v-if="isMenuOpen" class="lg:hidden bg-white border-b border-gray-100 shadow-lg max-h-[80vh] overflow-y-auto">
        <div class="px-4 pt-2 pb-6 space-y-1">
          <a href="#" @click="closeMenu"
            class="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-embassy-green hover:bg-green-50">
            {{ t('menu.home') }}
          </a>

          <div class="border-t border-gray-100 my-2 pt-2">
            <div class="px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">{{
              t('menu.ambassade.title') }}</div>
            <a v-for="item in embassyMenu" :key="item.key" :href="item.href" @click="closeMenu"
              class="block pl-6 pr-3 py-2 text-sm text-gray-600 hover:text-embassy-green">
              {{ t(item.label) }}
            </a>
          </div>
